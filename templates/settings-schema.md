...
| `reflect.enabled` | boolean | `true` | reflect-gate | Enable/disable per-task reflect-gate hints. Set to `false` to suppress all `[reflect-gate]` hints without restarting any agent. Read at fire time — takes effect immediately. |
