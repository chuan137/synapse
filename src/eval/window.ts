...
export function generateWindowReport(dbPath: string, opts: WindowReportOptions): string {
  ...
  // Window reports use the raw corpus (.synapse/evaluations/) for recency; fall back to tests/cases/

  const casesDir = existsSync(join(process.cwd(), '.synapse', 'evaluations'))
    ? join(process.cwd(), '.synapse', 'evaluations')
