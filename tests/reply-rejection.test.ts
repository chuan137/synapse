...
describe("synapse reply against a terminal row", () => {
  ...
  test("a rejected late reply does not bump the rev counter", () => {
    const db = freshDb();
    const { runId, taskId } = freshRun(db);
    const s1 = createSubtask(db, { runId, taskId, title: "a", assigneeRole: "coder", dependsOn: [] });
    failSubtask(db, s1, "exit 1"); // rev 2

    const before = db.query("SELECT rev_counter FROM runs WHERE id = ?").get(runId) as any;
    expect(() => replySubtask(db, s1, "late", null)).toThrow();
    const after = db.query("SELECT rev_counter FROM runs WHERE id = ?").get(runId) as any;
    expect(after.rev_counter).toBe(before.rev_counter);
  });
